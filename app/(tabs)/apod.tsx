...
export default function ApodScreen() {
  ...
  const buscarAPOD = async () => {
    ...
    try {
      ...
      const imgUrl = (data.hdurl || data.url || "").replace(
        /^http:\/\//i,
        "https://",
      );
      ...
      if (cachedFallback) {
        ...
        setApodDescricao(
          "Sem conexão com a internet. Abra o app com conexão para carregar a foto do dia.",
        );
        setModoOffline(true);
      }
    } finally {
      ...
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            Alert.alert(
              "✅ Cache limpo!",
              "O espaço foi liberado com sucesso.",
            );
          },
        },
      ],
      ...
      {showCacheInfo && (
        <View style={styles.cachePanel}>
          ...
          <Text style={styles.cachePanelInfo}>
            {cacheInfo.quantidade} imagens armazenadas · {cacheInfo.tamanhoMB}{" "}
            MB usados
          </Text>
          <TouchableOpacity
